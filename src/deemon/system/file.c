...
#include <stdint.h>  /* int64_t, uint8_t, uint32_t, uint64_t, uintptr_t */
...
PRIVATE struct type_getset tpconst debug_file_getsets[] = {
	TYPE_GETTER_F(STR_isatty, &debugfile_isatty, METHOD_FNOREFESCAPE, "->?Dbool"),
	TYPE_GETSET_END
};
...
err:
...
#else /* O_RDONLY == OPEN_FRDONLY && O_WRONLY == OPEN_FWRONLY && O_RDWR == OPEN_FRDWR */
	{
		PRIVATE int const accmode_flags[4] = {
			/* [Dee_OPEN_FRDONLY] = */ O_RDONLY,
			/* [Dee_OPEN_FWRONLY] = */ O_WRONLY,
			/* [Dee_OPEN_FRDWR  ] = */ O_RDWR,
			/* [Dee_OPEN_F???   ] = */ O_RDWR
		};
...
err:
	...
	result = (Dee_pos_t)lseek64((int)self->sf_handle, (int64_t)off, whence);
#else /* CONFIG_HAVE_lseek64 */
	result = (Dee_pos_t)lseek((int)self->sf_handle, (off_t)off, whence);
	...
	result = (Dee_pos_t)ftello64((FILE *)self->sf_handle);
	DBG_ALIGNMENT_ENABLE();
	if unlikely(result == (Dee_pos_t)-1)
		error_file_io(self);
	...
	result = (Dee_pos_t)ftello((FILE *)self->sf_handle);
	DBG_ALIGNMENT_ENABLE();
	if (result == (Dee_pos_t)-1)
		error_file_io(self);
