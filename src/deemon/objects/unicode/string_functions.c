...
/*[[[end]]]*/
		...
		goto retself;
	{
		...
		CASE_WIDTH_1BYTE:
			...
			findstr.cp8 = DeeString_As1Byte(args.find);
		...
		CASE_WIDTH_2BYTE:
			...
			if unlikely(!mystr.cp16)
				goto err_printer;
			findstr.cp16 = DeeString_As2Byte(args.find);
		...
		CASE_WIDTH_4BYTE:
			...
			if unlikely(!mystr.cp32)
				goto err_printer;
			findstr.cp32 = DeeString_As4Byte(args.find);
...
/*[[[end]]]*/
		...
		goto retself;
	{
		...
		CASE_WIDTH_1BYTE:
			...
			while ((ptr.cp8 = unicode_memcasememb(begin.cp8, end.cp8 - begin.cp8,
			                                      findstr.cp8, findlen,
			                                      &match_length)) != NULL) {
				/* Found one */
				if unlikely(Dee_unicode_printer_print8(&p, begin.cp8, (size_t)(ptr.cp8 - begin.cp8)) < 0)
		...
		CASE_WIDTH_2BYTE:
			...
			while ((ptr.cp16 = unicode_memcasememw(begin.cp16, end.cp16 - begin.cp16,
		...
		CASE_WIDTH_4BYTE:
			...
			while ((ptr.cp32 = unicode_memcasememl(begin.cp32, end.cp32 - begin.cp32,
...
string_bytes(String *self, size_t argc,
	...
	my_bytes = DeeString_AsBytes(self, allow_invalid);
...
/*[[[end]]]*/
	{
		...
		CASE_WIDTH_1BYTE:
			my_iter.cp8 = DeeString_Get1Byte(self);
		...
		CASE_WIDTH_2BYTE:
			my_iter.cp16 = DeeString_Get2Byte(self);
		...
		CASE_WIDTH_4BYTE:
			my_iter.cp32 = DeeString_Get4Byte(self);
...
string_findstr_impl(String *self, String *needle, size_t start, size_t end) {
	...
	CASE_WIDTH_2BYTE:
		lhs.cp16 = DeeString_As2Byte(self);
		if unlikely(!lhs.cp16)
			goto err;
		mylen = DeeString_WLEN(self);
		CLAMP_SUBSTR(&start, &end, &mylen, not_found);
		rhs.cp16 = DeeString_As2Byte(needle);
	...
	CASE_WIDTH_4BYTE:
		lhs.cp32 = DeeString_As4Byte(self);
		if unlikely(!lhs.cp32)
			goto err;
		mylen = WSTR_LENGTH(lhs.ptr);
		CLAMP_SUBSTR(&start, &end, &mylen, not_found);
		rhs.cp32 = DeeString_As4Byte(needle);
...
string_casefindstr_impl(String *self, String *needle, size_t start, size_t end,
	...
	CASE_WIDTH_1BYTE:
		...
		mylen   = WSTR_LENGTH(lhs.ptr);
		CLAMP_SUBSTR(&start, &end, &mylen, not_found);
		ptr.cp8 = unicode_memcasememb(lhs.cp8 + start, mylen,
		                              rhs.cp8, WSTR_LENGTH(rhs.cp8),
		                              &match_length);
		if (!ptr.cp8)
			goto not_found;
	...
	CASE_WIDTH_2BYTE:
		...
		mylen = WSTR_LENGTH(lhs.ptr);
		CLAMP_SUBSTR(&start, &end, &mylen, not_found);
		ptr.cp16 = unicode_memcasememw(lhs.cp16 + start, mylen,
		                               rhs.cp16, WSTR_LENGTH(rhs.cp16),
		                               &match_length);
		if (!ptr.cp16)
			goto not_found;
	...
	CASE_WIDTH_4BYTE:
		...
		mylen = WSTR_LENGTH(lhs.ptr);
		CLAMP_SUBSTR(&start, &end, &mylen, not_found);
		ptr.cp32 = unicode_memcasememl(lhs.cp32 + start, mylen,
		                               rhs.cp32, WSTR_LENGTH(rhs.cp32),
		                               &match_length);
		if (!ptr.cp32)
			goto not_found;
...
string_caserfindstr_impl(String *self, String *needle, size_t start, size_t end,
	...
	CASE_WIDTH_1BYTE:
		...
		mylen   = WSTR_LENGTH(lhs.ptr);
		CLAMP_SUBSTR(&start, &end, &mylen, not_found);
		ptr.cp8 = unicode_memcasermemb(lhs.cp8 + start, mylen,
		                               rhs.cp8, WSTR_LENGTH(rhs.cp8),
		                               &match_length);
		if (!ptr.cp8)
			goto not_found;
	...
	CASE_WIDTH_2BYTE:
		...
		mylen = WSTR_LENGTH(lhs.ptr);
		CLAMP_SUBSTR(&start, &end, &mylen, not_found);
		ptr.cp16 = unicode_memcasermemw(lhs.cp16 + start, mylen,
		                                rhs.cp16, WSTR_LENGTH(rhs.cp16),
		                                &match_length);
		if (!ptr.cp16)
			goto not_found;
	...
	CASE_WIDTH_4BYTE:
		...
		mylen = WSTR_LENGTH(lhs.ptr);
		CLAMP_SUBSTR(&start, &end, &mylen, not_found);
		ptr.cp32 = unicode_memcasermeml(lhs.cp32 + start, mylen,
		                                rhs.cp32, WSTR_LENGTH(rhs.cp32),
		                                &match_length);
		if (!ptr.cp32)
			goto not_found;
...
string_findany_cb(void *arg, DeeObject *elem) {
	...
	CASE_WIDTH_1BYTE:
		lhs.cp8 = DeeString_As1Byte(data->sfad_self);
		lhs.cp8 += data->sfad_base;
		rhs.cp8 = DeeString_As1Byte(elem);
	...
	CASE_WIDTH_2BYTE:
		lhs.cp16 = DeeString_As2Byte(data->sfad_self);
		if unlikely(!lhs.cp16)
			goto err;
		lhs.cp16 += data->sfad_base;
		rhs.cp16 = DeeString_As2Byte(elem);
		if unlikely(!rhs.cp16)
			goto err;
	...
	CASE_WIDTH_4BYTE:
		lhs.cp32 = DeeString_As4Byte(data->sfad_self);
		if unlikely(!lhs.cp32)
			goto err;
		lhs.cp32 += data->sfad_base;
		rhs.cp32 = DeeString_As4Byte(elem);
		if unlikely(!rhs.cp32)
			goto err;
...
string_rfindany_cb(void *arg, DeeObject *elem) {
	...
	CASE_WIDTH_1BYTE:
		lhs.cp8 = DeeString_As1Byte(data->srfad_self);
	...
	CASE_WIDTH_2BYTE:
		lhs.cp16 = DeeString_As2Byte(data->srfad_self);
		if unlikely(!lhs.cp16)
			goto err;
	...
	CASE_WIDTH_4BYTE:
		lhs.cp32 = DeeString_As4Byte(data->srfad_self);
		if unlikely(!lhs.cp32)
			goto err;
...
string_casefindany_cb(void *arg, DeeObject *elem) {
	...
	CASE_WIDTH_1BYTE:
		lhs.cp8 = DeeString_As1Byte(data->scfad_self);
		...
		ptr.cp8 = unicode_memcasememb(lhs.cp8, search_size,
		                              rhs.cp8, WSTR_LENGTH(rhs.cp8),
		                              &match_length);
		if (ptr.cp8 != NULL) {
			size_t hit = (size_t)(ptr.cp8 - lhs.cp8);
	...
	CASE_WIDTH_2BYTE:
		lhs.cp16 = DeeString_As2Byte(data->scfad_self);
		if unlikely(!lhs.cp16)
			goto err;
		...
		if (search_size > data->scfad_size)
			search_size = data->scfad_size;
		ptr.cp16 = unicode_memcasememw(lhs.cp16, search_size,
		                               rhs.cp16, WSTR_LENGTH(rhs.cp16),
		                               &match_length);
		if (ptr.cp16 != NULL) {
			size_t hit = (size_t)(ptr.cp16 - lhs.cp16);
	...
	CASE_WIDTH_4BYTE:
		lhs.cp32 = DeeString_As4Byte(data->scfad_self);
		if unlikely(!lhs.cp32)
			goto err;
		...
		if (search_size > data->scfad_size)
			search_size = data->scfad_size;
		ptr.cp32 = unicode_memcasememl(lhs.cp32, search_size,
		                               rhs.cp32, WSTR_LENGTH(rhs.cp32),
		                               &match_length);
		if (ptr.cp32 != NULL) {
			size_t hit = (size_t)(ptr.cp32 - lhs.cp32);
...
string_caserfindany_cb(void *arg, DeeObject *elem) {
	...
	CASE_WIDTH_1BYTE:
		lhs.cp8 = DeeString_As1Byte(data->scrfad_self);
		lhs.cp8 += data->scrfad_base;
		rhs.cp8 = DeeString_As1Byte(elem);
		ptr.cp8 = unicode_memcasermemb(lhs.cp8, data->scrfad_size,
		                               rhs.cp8, WSTR_LENGTH(rhs.cp8),
		                               &match_length);
		if (ptr.cp8 != NULL) {
			size_t hit = (size_t)(ptr.cp8 - lhs.cp8) + 1;
	...
	CASE_WIDTH_2BYTE:
		lhs.cp16 = DeeString_As2Byte(data->scrfad_self);
		if unlikely(!lhs.cp16)
			goto err;
		...
		if unlikely(!rhs.cp16)
			goto err;
		ptr.cp16 = unicode_memcasermemw(lhs.cp16, data->scrfad_size,
		                                rhs.cp16, WSTR_LENGTH(rhs.cp16),
		                                &match_length);
		if (ptr.cp16 != NULL) {
			size_t hit = (size_t)(ptr.cp16 - lhs.cp16) + 1;
	...
	CASE_WIDTH_4BYTE:
		lhs.cp32 = DeeString_As4Byte(data->scrfad_self);
		if unlikely(!lhs.cp32)
			goto err;
		...
		if unlikely(!rhs.cp32)
			goto err;
		ptr.cp32 = unicode_memcasermeml(lhs.cp32, data->scrfad_size,
		                                rhs.cp32, WSTR_LENGTH(rhs.cp32),
		                                &match_length);
		if (ptr.cp32 != NULL) {
			size_t hit = (size_t)(ptr.cp32 - lhs.cp32) + 1;
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		my_str.cp8 = DeeString_As1Byte(self);
		ot_str.cp8 = DeeString_As1Byte(args.needle);
	...
	CASE_WIDTH_2BYTE:
		my_str.cp16 = DeeString_As2Byte(self);
		if unlikely(!my_str.cp16)
			goto err;
		ot_str.cp16 = DeeString_As2Byte(args.needle);
	...
	CASE_WIDTH_4BYTE:
		my_str.cp32 = DeeString_As4Byte(self);
		if unlikely(!my_str.cp32)
			goto err;
		ot_str.cp32 = DeeString_As4Byte(args.needle);
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		...
		ot_len     = WSTR_LENGTH(ot_str.cp8);
		CLAMP_SUBSTR(&args.start, &args.end, &my_len, empty_range);
		return_bool(unicode_memcasestartswithb(my_str.cp8 + args.start, my_len, ot_str.cp8, ot_len));
	...
	CASE_WIDTH_2BYTE:
		...
		ot_len = WSTR_LENGTH(ot_str.cp16);
		CLAMP_SUBSTR(&args.start, &args.end, &my_len, empty_range);
		return_bool(unicode_memcasestartswithw(my_str.cp16 + args.start, my_len, ot_str.cp16, ot_len));
	...
	CASE_WIDTH_4BYTE:
		...
		ot_len = WSTR_LENGTH(ot_str.cp32);
		CLAMP_SUBSTR(&args.start, &args.end, &my_len, empty_range);
		return_bool(unicode_memcasestartswithl(my_str.cp32 + args.start, my_len, ot_str.cp32, ot_len));
	}
empty_range:
	return_bool(ot_len == 0);
err:
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		...
		ot_len     = WSTR_LENGTH(ot_str.cp8);
		CLAMP_SUBSTR(&args.start, &args.end, &my_len, empty_range);
		return_bool(unicode_memcaseendswithb(my_str.cp8 + args.start, my_len, ot_str.cp8, ot_len));
	...
	CASE_WIDTH_2BYTE:
		...
		ot_len = WSTR_LENGTH(ot_str.cp16);
		CLAMP_SUBSTR(&args.start, &args.end, &my_len, empty_range);
		return_bool(unicode_memcaseendswithw(my_str.cp16 + args.start, my_len, ot_str.cp16, ot_len));
	...
	CASE_WIDTH_4BYTE:
		...
		ot_len = WSTR_LENGTH(ot_str.cp32);
		CLAMP_SUBSTR(&args.start, &args.end, &my_len, empty_range);
		return_bool(unicode_memcaseendswithl(my_str.cp32 + args.start, my_len, ot_str.cp32, ot_len));
	}
empty_range:
	return_bool(ot_len == 0);
err:
...
/*[[[end]]]*/
	...
	if (args.filler) {
		...
		CASE_WIDTH_1BYTE:
			my_str.cp8 = DeeString_As1Byte(self);
			fl_str.cp8 = DeeString_As1Byte(args.filler);
			...
			if unlikely(!result)
				goto err;
			dst.cp8 = (uint8_t *)DeeString_As1Byte(result);
		...
		CASE_WIDTH_2BYTE:
			...
			if unlikely(!my_str.cp16)
				goto err;
			fl_str.cp16 = DeeString_As2Byte(args.filler);
		...
		CASE_WIDTH_4BYTE:
			...
			if unlikely(!my_str.cp32)
				goto err;
			fl_str.cp32 = DeeString_As4Byte(args.filler);
		...
		CASE_WIDTH_1BYTE:
			my_str.cp8 = DeeString_Get1Byte(self);
			result     = (DREF String *)DeeString_NewBuffer(args.width);
			if unlikely(!result)
				goto err;
			dst.cp8 = (uint8_t *)DeeString_As1Byte(result);
		...
		CASE_WIDTH_2BYTE:
			my_str.cp16 = DeeString_Get2Byte(self);
			dst.cp16 = buf.cp16 = DeeString_New2ByteBuffer(args.width);
			if unlikely(!buf.cp16)
				goto err;
		...
		CASE_WIDTH_4BYTE:
			my_str.cp32 = DeeString_Get4Byte(self);
			dst.cp32 = buf.cp32 = DeeString_New4ByteBuffer(args.width);
			if unlikely(!buf.cp32)
				goto err;
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		...
		lhs.cp8 += args.start;
		rhs.cp8 = DeeString_As1Byte(args.needle);
	...
	CASE_WIDTH_2BYTE:
		...
		CLAMP_SUBSTR(&args.start, &args.end, &mylen, not_found);
		lhs.cp16 += args.start;
		rhs.cp16 = DeeString_As2Byte(args.needle);
		if unlikely(!rhs.cp16)
			goto err;
	...
	CASE_WIDTH_4BYTE:
		...
		CLAMP_SUBSTR(&args.start, &args.end, &mylen, not_found);
		lhs.cp32 += args.start;
		rhs.cp32 = DeeString_As4Byte(args.needle);
		if unlikely(!rhs.cp32)
			goto err;
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		...
		lhs.cp8 += args.start;
		rhs.cp8 = DeeString_As1Byte(args.needle);
		result  = unicode_memcasecntb(lhs.cp8, mylen, rhs.cp8, WSTR_LENGTH(rhs.cp8));
		break;
	...
	CASE_WIDTH_2BYTE:
		...
		if unlikely(!rhs.cp16)
			goto err;
		result = unicode_memcasecntw(lhs.cp16, mylen, rhs.cp16, WSTR_LENGTH(rhs.cp16));
		break;
	...
	CASE_WIDTH_4BYTE:
		...
		if unlikely(!rhs.cp32)
			goto err;
		result = unicode_memcasecntl(lhs.cp32, mylen, rhs.cp32, WSTR_LENGTH(rhs.cp32));
		break;
	}
	return DeeInt_NewSize(result);
not_found:
...
string_casecontains_impl_f(String *self, String *needle,
	...
	CASE_WIDTH_1BYTE:
		...
		CLAMP_SUBSTR(&start, &end, &mylen, not_found);
		rhs.cp8 = DeeString_As1Byte(needle);
		ptr.cp8 = unicode_memcasememb(lhs.cp8 + start, mylen, rhs.cp8, WSTR_LENGTH(rhs.cp8), NULL);
		break;
	...
	CASE_WIDTH_2BYTE:
		...
		if unlikely(!rhs.cp16)
			goto err;
		ptr.cp16 = unicode_memcasememw(lhs.cp16 + start, mylen, rhs.cp16, WSTR_LENGTH(rhs.cp16), NULL);
		break;
	...
	CASE_WIDTH_4BYTE:
		...
		if unlikely(!rhs.cp32)
			goto err;
		ptr.cp32 = unicode_memcasememl(lhs.cp32 + start, mylen, rhs.cp32, WSTR_LENGTH(rhs.cp32), NULL);
		break;
	}
	if (!ptr.ptr)
		goto not_found;
...
partition_pack_notfoundb(String *__restrict self,
	...
	if unlikely(!result)
		goto err;
	if (lhs == DeeString_Get1Byte(self) &&
...
partition_pack_notfoundw(String *__restrict self,
	...
	if unlikely(!result)
		goto err;
	if (lhs == DeeString_Get2Byte(self) &&
	    WSTR_LENGTH(lhs) == lhs_length) {
		lhs_string = self;
		Dee_Incref(self);
	} else {
...
partition_pack_notfoundl(String *__restrict self,
	...
	if unlikely(!result)
		goto err;
	if (lhs == DeeString_Get4Byte(self) &&
	    WSTR_LENGTH(lhs) == lhs_length) {
		lhs_string = self;
		Dee_Incref(self);
	} else {
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		...
		lhs.cp8 += args.start;
		rhs.cp8 = DeeString_As1Byte(args.needle);
		ptr.cp8 = unicode_memcasememb(lhs.cp8, mylen,
		                              rhs.cp8, WSTR_LENGTH(rhs.cp8),
		                              &match_length);
		if (!ptr.cp8)
			goto not_foundb;
...
not_foundb:
	...
	CASE_WIDTH_2BYTE:
		...
		if unlikely(!rhs.cp16)
			goto err;
		ptr.cp16 = unicode_memcasememw(lhs.cp16, mylen,
		                               rhs.cp16, WSTR_LENGTH(rhs.cp16),
		                               &match_length);
		if (!ptr.cp16)
			goto not_foundw;
...
not_foundw:
	...
	CASE_WIDTH_4BYTE:
		...
		if unlikely(!rhs.cp32)
			goto err;
		ptr.cp32 = unicode_memcasememl(lhs.cp32, mylen,
		                               rhs.cp32, WSTR_LENGTH(rhs.cp32),
		                               &match_length);
		if (!ptr.cp32)
			goto not_foundl;
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		...
		lhs.cp8 += args.start;
		rhs.cp8 = DeeString_As1Byte(args.needle);
		ptr.cp8 = unicode_memcasermemb(lhs.cp8, mylen,
		                               rhs.cp8, WSTR_LENGTH(rhs.cp8),
		                               &match_length);
		if (!ptr.cp8)
			goto not_foundb;
...
not_foundb:
	...
	CASE_WIDTH_2BYTE:
		...
		if unlikely(!rhs.cp16)
			goto err;
		ptr.cp16 = unicode_memcasermemw(lhs.cp16, mylen,
		                                rhs.cp16, WSTR_LENGTH(rhs.cp16),
		                                &match_length);
		if (!ptr.cp16)
			goto not_foundw;
...
not_foundw:
	...
	CASE_WIDTH_4BYTE:
		...
		if unlikely(!rhs.cp32)
			goto err;
		ptr.cp32 = unicode_memcasermeml(lhs.cp32, mylen,
		                                rhs.cp32, WSTR_LENGTH(rhs.cp32),
		                                &match_length);
		if (!ptr.cp32)
			goto not_foundl;
...
#define DO_COMPARE(lhs_bwl, lhs_str, lhs_len, rhs_bwl, rhs_str, rhs_len) \
	...
	if (!lhs->s_data ||
		...
		if (!rhs->s_data ||
			...
			/* Most simple case: compare ascii/single-byte strings. */
			return unicode_memcasecmpb(lhs_str, lhs_len,
...
string_vercompare(String *self, size_t argc, DeeObject *const *argv) {
	...
	CASE_WIDTH_2BYTE:
		...
		if unlikely(!my_str.cp16)
			goto err;
		ot_str.cp16 = DeeString_As2Byte(args.other);
		if unlikely(!ot_str.cp16)
			goto err;
		my_len = WSTR_LENGTH(my_str.cp16);
		ot_len = WSTR_LENGTH(ot_str.cp16);
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_strverscmpw(my_str.cp16 + args.my_start, my_len,
	...
	CASE_WIDTH_4BYTE:
		...
		if unlikely(!my_str.cp32)
			goto err;
		ot_str.cp32 = DeeString_As4Byte(args.other);
		if unlikely(!ot_str.cp32)
			goto err;
		my_len = WSTR_LENGTH(my_str.cp32);
		ot_len = WSTR_LENGTH(ot_str.cp32);
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_strverscmpl(my_str.cp32 + args.my_start, my_len,
...
string_casevercompare(String *self, size_t argc, DeeObject *const *argv) {
	...
	CASE_WIDTH_1BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_strcaseverscmpb(my_str.cp8 + args.my_start, my_len,
		                                 ot_str.cp8 + args.ot_start, ot_len);
		break;
	...
	CASE_WIDTH_2BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_strcaseverscmpw(my_str.cp16 + args.my_start, my_len,
		                                 ot_str.cp16 + args.ot_start, ot_len);
		break;
	...
	CASE_WIDTH_4BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_strcaseverscmpl(my_str.cp32 + args.my_start, my_len,
		                                 ot_str.cp32 + args.ot_start, ot_len);
		break;
	}
	return_reference(DeeInt_FromSign(result));
...
string_fuzzycompare(String *self, size_t argc, DeeObject *const *argv) {
	...
	CASE_WIDTH_1BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_fuzzy_compareb(my_str.cp8 + args.my_start, my_len,
		                                ot_str.cp8 + args.ot_start, ot_len);
		break;
	...
	CASE_WIDTH_2BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_fuzzy_comparew(my_str.cp16 + args.my_start, my_len,
		                                ot_str.cp16 + args.ot_start, ot_len);
		break;
	...
	CASE_WIDTH_4BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_fuzzy_comparel(my_str.cp32 + args.my_start, my_len,
		                                ot_str.cp32 + args.ot_start, ot_len);
		break;
	}
	if unlikely(result == (size_t)-1)
...
string_casefuzzycompare(String *self, size_t argc, DeeObject *const *argv) {
	...
	CASE_WIDTH_1BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_fuzzy_casecompareb(my_str.cp8 + args.my_start, my_len,
		                                    ot_str.cp8 + args.ot_start, ot_len);
		break;
	...
	CASE_WIDTH_2BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_fuzzy_casecomparew(my_str.cp16 + args.my_start, my_len,
		                                    ot_str.cp16 + args.ot_start, ot_len);
		break;
	...
	CASE_WIDTH_4BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_fuzzy_casecomparel(my_str.cp32 + args.my_start, my_len,
		                                    ot_str.cp32 + args.ot_start, ot_len);
		break;
	}
	if unlikely(result == (size_t)-1)
...
string_wildcompare(String *self, size_t argc, DeeObject *const *argv) {
	...
	CASE_WIDTH_1BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_wildcompareb(my_str.cp8 + args.my_start, my_len,
		                              ot_str.cp8 + args.ot_start, ot_len);
		break;
	...
	CASE_WIDTH_2BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_wildcomparew(my_str.cp16 + args.my_start, my_len,
		                              ot_str.cp16 + args.ot_start, ot_len);
		break;
	...
	CASE_WIDTH_4BYTE:
		...
		CLAMP_SUBSTR_IMPLICIT(&args.my_start, &args.my_end, &my_len);
		CLAMP_SUBSTR_IMPLICIT(&args.ot_start, &args.ot_end, &ot_len);
		result = unicode_wildcomparel(my_str.cp32 + args.my_start, my_len,
		                              ot_str.cp32 + args.ot_start, ot_len);
		break;
	}
	return_reference(DeeInt_FromSign(result));
...
string_casewildcompare(String *self, size_t argc, DeeObject *const *argv) {
	...
	CASE_WIDTH_1BYTE:
		...
		unicode_foldreader_initb(&my_reader, my_str.cp8 + args.my_start, my_len);
		unicode_foldreader_initb(&ot_reader, ot_str.cp8 + args.ot_start, ot_len);
		result = unicode_wildcasecompareb(&my_reader, &ot_reader);
		break;
	...
	CASE_WIDTH_2BYTE:
		...
		unicode_foldreader_initw(&my_reader, my_str.cp16 + args.my_start, my_len);
		unicode_foldreader_initw(&ot_reader, ot_str.cp16 + args.ot_start, ot_len);
		result = unicode_wildcasecomparew(&my_reader, &ot_reader);
		break;
	...
	CASE_WIDTH_4BYTE:
		...
		unicode_foldreader_initl(&my_reader, my_str.cp32 + args.my_start, my_len);
		unicode_foldreader_initl(&ot_reader, ot_str.cp32 + args.ot_start, ot_len);
		result = unicode_wildcasecomparel(&my_reader, &ot_reader);
		break;
	}
	return_reference(DeeInt_FromSign(result));
err:
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		scan_str.cp8 = DeeString_As1Byte(self);
		open_str.cp8 = DeeString_As1Byte(args.open);
		clos_str.cp8 = DeeString_As1Byte(args.close);
		...
		ptr.cp8 = unicode_find_matchb(scan_str.cp8 + args.start, scan_len,
	...
	CASE_WIDTH_2BYTE:
		scan_str.cp16 = DeeString_As2Byte(self);
		if unlikely(!scan_str.cp16)
			goto err;
		open_str.cp16 = DeeString_As2Byte(args.open);
		if unlikely(!open_str.cp16)
			goto err;
		clos_str.cp16 = DeeString_As2Byte(args.close);
		...
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp16 = unicode_find_matchw(scan_str.cp16 + args.start, scan_len,
	...
	CASE_WIDTH_4BYTE:
		scan_str.cp32 = DeeString_As4Byte(self);
		if unlikely(!scan_str.cp32)
			goto err;
		open_str.cp32 = DeeString_As4Byte(args.open);
		if unlikely(!open_str.cp32)
			goto err;
		clos_str.cp32 = DeeString_As4Byte(args.close);
		...
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp32 = unicode_find_matchl(scan_str.cp32 + args.start, scan_len,
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		...
		scan_len     = WSTR_LENGTH(scan_str.cp8);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp8 = unicode_find_casematchb(scan_str.cp8 + args.start, scan_len,
		                                  open_str.cp8, open_len,
		                                  clos_str.cp8, clos_len,
		                                  &match_length);
		if unlikely(!ptr.cp8)
	...
	CASE_WIDTH_2BYTE:
		...
		scan_len = WSTR_LENGTH(scan_str.cp16);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp16 = unicode_find_casematchw(scan_str.cp16 + args.start, scan_len,
		                                   open_str.cp16, open_len,
		                                   clos_str.cp16, clos_len,
		                                   &match_length);
		if unlikely(!ptr.cp16)
	...
	CASE_WIDTH_4BYTE:
		...
		scan_len = WSTR_LENGTH(scan_str.cp32);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp32 = unicode_find_casematchl(scan_str.cp32 + args.start, scan_len,
		                                   open_str.cp32, open_len,
		                                   clos_str.cp32, clos_len,
		                                   &match_length);
		if unlikely(!ptr.cp32)
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		...
		scan_len     = WSTR_LENGTH(scan_str.cp8);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp8 = unicode_rfind_matchb(scan_str.cp8 + args.start, scan_len,
		                               open_str.cp8, open_len,
		                               clos_str.cp8, clos_len);
		if unlikely(!ptr.cp8)
			goto not_found;
	...
	CASE_WIDTH_2BYTE:
		...
		scan_len = WSTR_LENGTH(scan_str.cp16);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp16 = unicode_rfind_matchw(scan_str.cp16 + args.start, scan_len,
		                                open_str.cp16, open_len,
		                                clos_str.cp16, clos_len);
		if unlikely(!ptr.cp16)
			goto not_found;
	...
	CASE_WIDTH_4BYTE:
		...
		scan_len = WSTR_LENGTH(scan_str.cp32);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp32 = unicode_rfind_matchl(scan_str.cp32 + args.start, scan_len,
		                                open_str.cp32, open_len,
		                                clos_str.cp32, clos_len);
		if unlikely(!ptr.cp32)
			goto not_found;
...
/*[[[end]]]*/
	...
	CASE_WIDTH_1BYTE:
		...
		scan_len     = WSTR_LENGTH(scan_str.cp8);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp8 = unicode_rfind_casematchb(scan_str.cp8 + args.start, scan_len,
		                                   open_str.cp8, open_len,
		                                   clos_str.cp8, clos_len,
		                                   &match_length);
		if unlikely(!ptr.cp8)
	...
	CASE_WIDTH_2BYTE:
		...
		scan_len = WSTR_LENGTH(scan_str.cp16);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp16 = unicode_rfind_casematchw(scan_str.cp16 + args.start, scan_len,
		                                    open_str.cp16, open_len,
		                                    clos_str.cp16, clos_len,
		                                    &match_length);
		if unlikely(!ptr.cp16)
	...
	CASE_WIDTH_4BYTE:
		...
		scan_len = WSTR_LENGTH(scan_str.cp32);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		ptr.cp32 = unicode_rfind_casematchl(scan_str.cp32 + args.start, scan_len,
		                                    open_str.cp32, open_len,
		                                    clos_str.cp32, clos_len,
		                                    &match_length);
		if unlikely(!ptr.cp32)
...
#define SET_STRING(a, b, c)                                \
	...
	CASE_WIDTH_1BYTE:
		...
		if unlikely(!match_start.cp8)
			goto not_found;
		match_end.cp8 = unicode_find_matchb(match_start.cp8 + open_len,
	...
	CASE_WIDTH_2BYTE:
		...
		if unlikely(!match_start.cp16)
			goto not_found;
		match_end.cp16 = unicode_find_matchw(match_start.cp16 + open_len,
	...
	CASE_WIDTH_4BYTE:
		...
		if unlikely(!match_start.cp32)
			goto not_found;
		match_end.cp32 = unicode_find_matchl(match_start.cp32 + open_len,
...
#define SET_STRING(a, b, c)                                \
	...
	CASE_WIDTH_1BYTE:
		...
		if unlikely(!match_end.cp8)
			goto not_found;
		match_start.cp8 = unicode_rfind_matchb(scan_str.cp8 + args.start,
	...
	CASE_WIDTH_2BYTE:
		...
		if unlikely(!match_end.cp16)
			goto not_found;
		match_start.cp16 = unicode_rfind_matchw(scan_str.cp16 + args.start,
	...
	CASE_WIDTH_4BYTE:
		...
		if unlikely(!match_end.cp32)
			goto not_found;
		match_start.cp32 = unicode_rfind_matchl(scan_str.cp32 + args.start,
...
#define SET_STRING(a, b, c)                                \
	...
	CASE_WIDTH_1BYTE:
		...
		scan_len     = WSTR_LENGTH(scan_str.cp8);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		match_start.cp8 = unicode_memcasememb(scan_str.cp8 + args.start, scan_len,
		                                      open_str.cp8, open_len,
		                                      &match_length);
		if unlikely(!match_start.cp8)
			goto not_found;
		match_end.cp8 = unicode_find_casematchb(match_start.cp8 + match_length,
		                                        scan_len - (match_start.cp8 - (scan_str.cp8 + args.start)),
		                                        open_str.cp8, open_len,
		                                        clos_str.cp8, clos_len,
		                                        &match_length);
	...
	CASE_WIDTH_2BYTE:
		...
		match_start.cp16 = unicode_memcasememw(scan_str.cp16 + args.start, scan_len,
		                                       open_str.cp16, open_len,
		                                       &match_length);
		if unlikely(!match_start.cp16)
			goto not_found;
		match_end.cp16 = unicode_find_casematchw(match_start.cp16 + match_length,
		                                         scan_len - (match_start.cp16 - (scan_str.cp16 + args.start)),
		                                         open_str.cp16, open_len,
		                                         clos_str.cp16, clos_len,
		                                         &match_length);
	...
	CASE_WIDTH_4BYTE:
		...
		scan_len = WSTR_LENGTH(scan_str.cp32);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		match_start.cp32 = unicode_memcasememl(scan_str.cp32 + args.start, scan_len,
		                                       open_str.cp32, open_len,
		                                       &match_length);
		if unlikely(!match_start.cp32)
			goto not_found;
		match_end.cp32 = unicode_find_casematchl(match_start.cp32 + match_length,
		                                         scan_len - (match_start.cp32 - (scan_str.cp32 + args.start)),
		                                         open_str.cp32, open_len,
		                                         clos_str.cp32, clos_len,
		                                         &match_length);
...
#define SET_STRING(a, b, c)                                \
	...
	CASE_WIDTH_1BYTE:
		...
		scan_len     = WSTR_LENGTH(scan_str.cp8);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		match_end.cp8 = unicode_memcasermemb(scan_str.cp8 + args.start, scan_len,
		                                     clos_str.cp8, clos_len, NULL);
		if unlikely(!match_end.cp8)
			goto not_found;
		match_start.cp8 = unicode_rfind_casematchb(scan_str.cp8 + args.start,
		                                           (size_t)(match_end.cp8 - (scan_str.cp8 + args.start)),
		                                           open_str.cp8, open_len,
		                                           clos_str.cp8, clos_len, NULL);
		if unlikely(!match_start.cp8)
	...
	CASE_WIDTH_2BYTE:
		...
		scan_len = WSTR_LENGTH(scan_str.cp16);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		match_end.cp16 = unicode_memcasermemw(scan_str.cp16 + args.start, scan_len,
		                                      clos_str.cp16, clos_len, NULL);
		if unlikely(!match_end.cp16)
			goto not_found;
		match_start.cp16 = unicode_rfind_casematchw(scan_str.cp16 + args.start,
		                                            (size_t)(match_end.cp16 - (scan_str.cp16 + args.start)),
		                                            open_str.cp16, open_len,
		                                            clos_str.cp16, clos_len, NULL);
		if unlikely(!match_start.cp16)
	...
	CASE_WIDTH_4BYTE:
		...
		scan_len = WSTR_LENGTH(scan_str.cp32);
		CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
		match_end.cp32 = unicode_memcasermeml(scan_str.cp32 + args.start, scan_len,
		                                      clos_str.cp32, clos_len, NULL);
		if unlikely(!match_end.cp32)
			goto not_found;
		match_start.cp32 = unicode_rfind_casematchl(scan_str.cp32 + args.start,
		                                            (size_t)(match_end.cp32 - (scan_str.cp32 + args.start)),
		                                            open_str.cp32, open_len,
		                                            clos_str.cp32, clos_len, NULL);
		if unlikely(!match_start.cp32)
...
generic_regex_getargs(String *self, size_t argc, DeeObject *const *argv,
	...
	result->rx_inbase = DeeString_AsUtf8(self);
...
search_regex_getargs(String *self, size_t argc, DeeObject *const *argv,
	...
	result->rewr_exec.rx_inbase = DeeString_AsUtf8(self);
...
/*[[[end]]]*/
	...
	exec.rx_inbase = DeeString_AsUtf8(self);
...
INTERN_TPCONST struct type_method tpconst string_methods[] = {
	...
	TYPE_KWMETHOD("encode", &string_encode,
	              ...
	              "If the given @codec is not apart of this list, a call is made to ?Eiconv:encode"),
...
string_mod(String *__restrict self, DeeObject *__restrict args) {
	...
	format_str = DeeString_AsUtf8(self);
...
string_contains(String *self, DeeObject *some_object) {
	...
	if (DeeBytes_Check(some_object)) {
		...
		CASE_WIDTH_1BYTE:
			str.cp8   = DeeString_As1Byte(self);
			other.cp8 = DeeString_As1Byte(some_object);
			ptr.cp8 = memmemb(str.cp8, WSTR_LENGTH(str.cp8),
			                  other.cp8, WSTR_LENGTH(other.cp8));
			break;

		CASE_WIDTH_2BYTE:
			str.cp16 = DeeString_As2Byte(self);
			if unlikely(!str.cp16)
				goto err;
			other.cp16 = DeeString_As2Byte(some_object);
		...
		CASE_WIDTH_4BYTE:
			str.cp32 = DeeString_As4Byte(self);
			if unlikely(!str.cp32)
				goto err;
			other.cp32 = DeeString_As4Byte(some_object);
