...
/*[[[end]]]*/
	...
	if (data.bcrfad_base > orig_base || status == -2) {
		...
		return DeeSeq_OfPairII(index, index + data.bcrfad_reslen);
	}
not_found:
	return_none;
err:
...
bytes_vercompare(Bytes *self, size_t argc, DeeObject *const *argv) {
	...
	if (acquire_bcompare_args(self, &args, argc, argv, "vercompare"))
		goto err;
	result = unicode_strverscmpb(args.lhs_ptr, args.lhs_len,
...
bytes_wildcompare(Bytes *self, size_t argc, DeeObject *const *argv) {
	...
	if (acquire_bcompare_args(self, &args, argc, argv, "wildcompare"))
		goto err;
	result = unicode_wildcompareb(args.lhs_ptr, args.lhs_len,
	                              args.rhs_ptr, args.rhs_len);
	release_bcompare_args(self, &args);
	return_reference(DeeInt_FromSign(result));
err:
...
bytes_fuzzycompare(Bytes *self, size_t argc, DeeObject *const *argv) {
	...
	if (acquire_bcompare_args(self, &args, argc, argv, "fuzzycompare"))
		goto err;
	result = unicode_fuzzy_compareb(args.lhs_ptr, args.lhs_len,
	                                args.rhs_ptr, args.rhs_len);
	release_bcompare_args(self, &args);
...
bytes_casevercompare(Bytes *self, size_t argc, DeeObject *const *argv) {
	...
	if (acquire_bcompare_args(self, &args, argc, argv, "casevercompare"))
		goto err;
	result = unicode_strcaseverscmp_ascii(args.lhs_ptr, args.lhs_len,
	                                      args.rhs_ptr, args.rhs_len);
	release_bcompare_args(self, &args);
	return_reference(DeeInt_FromSign(result));
err:
...
bytes_casewildcompare(Bytes *self, size_t argc, DeeObject *const *argv) {
	...
	if (acquire_bcompare_args(self, &args, argc, argv, "casewildcompare"))
		goto err;
	result = unicode_wildcasecompare_ascii(args.lhs_ptr, args.lhs_len,
	                                       args.rhs_ptr, args.rhs_len);
	release_bcompare_args(self, &args);
	return_reference(DeeInt_FromSign(result));
err:
...
bytes_casefuzzycompare(Bytes *self, size_t argc, DeeObject *const *argv) {
	...
	if (acquire_bcompare_args(self, &args, argc, argv, "casefuzzycompare"))
		goto err;
	result = unicode_fuzzy_casecompare_ascii(args.lhs_ptr, args.lhs_len,
	                                     args.rhs_ptr, args.rhs_len);
	release_bcompare_args(self, &args);
...
/*[[[end]]]*/
	...
	ptr = unicode_find_matchb(scan_str + args.start, scan_len,
...
/*[[[end]]]*/
	...
	CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
	scan_str = DeeBytes_DATA(self);
	ptr = unicode_rfind_matchb(scan_str + args.start, scan_len,
	                           s_open.n_data, s_open.n_size,
	                           s_clos.n_data, s_clos.n_size);
	if unlikely(!ptr)
		goto not_found;
...
/*[[[end]]]*/
	...
	CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
	scan_str = DeeBytes_DATA(self);
	ptr = unicode_find_casematch_ascii(scan_str + args.start, scan_len,
	                                   s_open.n_data, s_open.n_size,
	                                   s_clos.n_data, s_clos.n_size);
	if unlikely(!ptr)
		goto not_found;
	return DeeSeq_OfPairII((size_t)(ptr - scan_str), s_clos.n_size);
not_found:
	return_none;
err:
	return NULL;
...
/*[[[end]]]*/
	...
	CLAMP_SUBSTR(&args.start, &args.end, &scan_len, not_found);
	scan_str = DeeBytes_DATA(self);
	ptr = unicode_rfind_casematch_ascii(scan_str + args.start, scan_len,
	                                    s_open.n_data, s_open.n_size,
	                                    s_clos.n_data, s_clos.n_size);
	if unlikely(!ptr)
		goto not_found;
	result = (size_t)(ptr - scan_str);
	return DeeSeq_OfPairII(result, result + s_open.n_size);
not_found:
	return_none;
err:
	return NULL;
...
/*[[[end]]]*/
	...
	if unlikely(!ptr)
		goto not_found;
	result = (size_t)(ptr - scan_str);
	return DeeSeq_OfPairII(result, result + s_clos.n_size);
not_found:
	DeeRT_ErrSubstringNotFound(Dee_AsObject(self), args.close, args.start, args.end);
err:
	return NULL;
...
#define SET_BYTES(a, b, c)                                 \
	...
	if unlikely(!match_start)
		goto not_found;
	match_end = (byte_t *)unicode_find_matchb(match_start + s_open.n_size,
...
#define SET_BYTES(a, b, c)                                 \
	...
	if unlikely(!match_end)
		goto not_found;
	match_start = (byte_t *)unicode_rfind_matchb(scan_str + args.start,
...
#define SET_BYTES(a, b, c)                                 \
	...
	if unlikely(!match_start)
		goto not_found;
	match_end = (byte_t *)unicode_find_casematch_ascii(match_start + s_open.n_size,
	                                                   scan_len - (match_start - (scan_str + args.start)),
	                                                   s_open.n_data, s_open.n_size,
	                                                   s_clos.n_data, s_clos.n_size);
	if unlikely(!match_end)
...
#define SET_BYTES(a, b, c)                                 \
	...
	if unlikely(!match_end)
		goto not_found;
	match_start = (byte_t *)unicode_rfind_casematch_ascii(scan_str + args.start,
	                                                      (size_t)(match_end - (scan_str + args.start)),
	                                                      s_open.n_data, s_open.n_size,
	                                                      s_clos.n_data, s_clos.n_size);
	if unlikely(!match_start)
