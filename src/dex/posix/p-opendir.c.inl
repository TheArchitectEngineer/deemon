...
#include <deemon/alloc.h>           /* DeeObject_FREE, DeeObject_MALLOC, Dee_Freea, Dee_Malloca, Dee_TYPE_CONSTRUCTOR_INIT_FIXED */
...
#include <deemon/object.h>          /* DREF, DeeObject, DeeObject_AsUInt, DeeObject_AsUIntX, DeeTypeObject, Dee_AsObject, Dee_Decref, Dee_Decref_likely, Dee_Incref, Dee_TYPE, Dee_XDecref, ITER_DONE, OBJECT_HEAD, OBJECT_HEAD_INIT, return_reference_ */
...
again_FindFirstFileExW:
	...
	if unlikely(self->odi_hnd == INVALID_HANDLE_VALUE) {
		...
		if (dwError != ERROR_NO_MORE_FILES) {
			DeeNTSystem_HandleGenericError(dwError, err_wpattern, again_FindFirstFileExW);
...
again_skipdots:
				...
				if (!FindNextFileW(self->odi_hnd, &self->odi_data)) {
					...
					if unlikely(dwError != ERROR_NO_MORE_FILES) {
						DeeNTSystem_HandleGenericError(dwError, err_FindClose, again_skipdots);
						DeeNTSystem_ThrowErrorf(&DeeError_FSError, dwError,
...
again_GetFileInformationByHandle:
		...
		dwError = GetLastError();
		DBG_ALIGNMENT_ENABLE();
		DeeNTSystem_HandleGenericError(dwError, err_fullname_hFile, again_GetFileInformationByHandle);
